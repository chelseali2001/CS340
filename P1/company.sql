...
INSERT INTO DEPT_STATS 
  SELECT d.Dnumber, (SELECT COUNT(e.Dno) FROM EMPLOYEE e WHERE e.Dno=d.Dnumber) AS `Emp_Count`, (SELECT AVG(e.Salary) FROM EMPLOYEE e WHERE e.Dno=d.Dnumber) AS `Avg_salary`
  FROM DEPARTMENT d;
END $$
...
BEGIN
	IF OLD.Dno IS NOT NULL THEN    
		...
		UPDATE DEPT_STATS
    	SET Emp_Count = Emp_Count - 1, Avg_Salary = (SELECT ((SUM(Salary) - OLD.Salary) / Emp_Count) FROM EMPLOYEE WHERE Dno=OLD.Dno)
...
BEGIN
    ...
    ELSE
		INSERT INTO DEPT_STATS VALUES
        (NEW.Dno, 1, NEW.Salary);
    END IF;
END;
$$
...
BEGIN
	IF OLD.Dno IS NOT NULL THEN
		UPDATE DEPT_STATS
    	SET Avg_Salary = (SELECT AVG(Salary) FROM EMPLOYEE WHERE Dno=OLD.Dno)
    	WHERE DEPT_STATS.Dnumber=OLD.Dno;
    END IF;
    
    IF (NEW.Dno IS NOT NULL) THEN
    	UPDATE DEPT_STATS
        SET Avg_Salary = (SELECT AVG(Salary) FROM EMPLOYEE WHERE Dno=NEW.Dno)
        WHERE DEPT_STATS.Dnumber=NEW.Dno;
    END IF;
END;
$$
...
FOR EACH ROW
BEGIN
	  DECLARE themessage VARCHAR(100);
    DECLARE totalHours INT(2);
    DECLARE currentHours INT(2);
    SELECT SUM(Hours) + NEW.Hours INTO totalHours FROM WORKS_ON WHERE Essn=NEW.Essn;
    SELECT SUM(Hours) INTO currentHours FROM WORKS_ON WHERE Essn=NEW.Essn;
    
	IF (totalHours > 40) THEN
    	SET themessage = concat('You entered ', NEW.Hours, '. You currently work ', currentHours, '. You are over 40 hours.');
        SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = themessage;
    ELSEIF (NEW.Hours > 40) THEN
    	SET themessage = concat('You cannot work ', NEW.Hours, '. You can are over 40 hours.');
        SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = themessage;
    END IF;
END;
$$
...
CREATE FUNCTION `PayLevel` (eSsn int)
RETURNS VARCHAR(100)
BEGIN
  DECLARE amount decimal(10, 2);
  DECLARE average decimal(10, 2);
  SELECT Salary INTO amount FROM EMPLOYEE WHERE Ssn=eSsn;
  SELECT AVG(Salary) INTO average FROM EMPLOYEE WHERE Dno=(SELECT Dno FROM EMPLOYEE WHERE Ssn=eSsn);
